...
추천받은 학생의 사진을 사진틀에 게시하고 추천받은 횟수를 표시하는 규칙은 다음과 같다.
 ...
 이때, 현재까지 추천 받은 횟수가 가장 적은 학생이 두 명 이상일 경우에는 게시된 지 가장 오래된 사진을 삭제한다.
        ...
        for(int i=1; i<=R; i++){
            ...
            if(num[std]++ == 0){
                ...
                if(full < N){
                    ...
                    idx = 0;
                    for(int j=1; j<N; j++)
                        if(num[frame[idx]] > num[frame[j]] || (num[frame[idx]] == num[frame[j]] && order[frame[idx]] > order[frame[j]]))
                            idx = j;
                    num[frame[idx]] = 0;
                    frame[idx] = std;
                }
