...
public class NetConnect {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0;i<M;i++){
            ...
            pQueue.add(new Edge(Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken())));
        }
        int ans = 0;
        int cnt = 0;
        while(cnt < N-1){
    ...
    public static int find(int n){
        if(parent[n] == n)
            return n;
        parent[n] = find(parent[n]); // 최상위부모로 설정 (성능 향상)
        return parent[n];
    }
