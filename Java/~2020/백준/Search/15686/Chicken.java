...
public class Chicken {
    public static ArrayList<Integer> com = new ArrayList<>();
    ...
    public static void choice(int m, int idx, int s, int h, Tuple3[] S, Tuple3[] H){
        if(com.size() == m){
            ...
            for(int i=0; i<h; i++){
                ...
                for(int j=0; j<m; j++){
                    if(H[i] != null && S[com.get(j)] != null)
                        dis = Math.min(dis, Math.abs(H[i].r-S[com.get(j)].r)+Math.abs(H[i].c-S[com.get(j)].c));
                }
                tot += dis;
            }
