...
public class MakePW {
    ...
    public static void find(int idx, int i, int l, int c) throws IOException {
        ans[i++] = ch[idx++];
        if(i != l){
            for(int j=idx; j<=c-(l-i); j++)
                find(j, i, l, c);
        }else{
            int vow=0, con=0;
            for(int j=0; j<l; j++){
                if(vow < 1 || con < 2){
                    if(ans[j].equals("a") || ans[j].equals("e") || ans[j].equals("i") || ans[j].equals("o") || ans[j].equals("u"))
                        vow++;
                    else
                        con++;
                }else
